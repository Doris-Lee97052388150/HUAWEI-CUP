...
def plot_q3_routes(decoded: DecodeResult,nodes:Dict[str,Node],sorties:List[RelaySortie],out:Path)->None:
    fig,ax=plt.subplots(figsize=(10,8))
    # 运输路线：只给第一条设置 legend label，避免图例重复。
    for k,t in enumerate(decoded.trips):
        seq=["O01"]+t.service_sequence+["O01"]
        ax.plot([nodes[n].lon for n in seq],[nodes[n].lat for n in seq],
                alpha=.38,linewidth=1,label="Transport route" if k==0 else None)

    # 调度中心与服务区采用不同 marker；中继悬停点使用三角形。
    o=nodes["O01"]
    ax.scatter([o.lon],[o.lat],s=72,marker="s",label="O01 dispatch center",zorder=5)
    ax.text(o.lon,o.lat,"O01",fontsize=8,fontweight="bold")

    service_nodes=[n for nid,n in nodes.items() if nid!="O01"]
    ax.scatter([n.lon for n in service_nodes],[n.lat for n in service_nodes],
               s=24,marker="o",label="Service area",zorder=4)
    for n in service_nodes:
        ax.text(n.lon,n.lat,n.node_id,fontsize=7)

    if sorties:
        # 同一悬停位置可能对应多个架次；点全部画出，但图例只保留一个类别。
        for k,srt in enumerate(sorties):
            ax.scatter(srt.lon,srt.lat,marker="^",s=78,
                       label="Relay hover point" if k==0 else None,zorder=6)
            ax.text(srt.lon,srt.lat,srt.sortie_id,fontsize=7)
        # 用虚线表示 O01 到中继悬停点的部署方向（示意），帮助区分运输路线。
        for k,srt in enumerate(sorties):
            ax.plot([o.lon,srt.lon],[o.lat,srt.lat],linestyle="--",linewidth=.75,alpha=.28,
                    label="Relay deployment" if k==0 else None)

    ax.set_title("Q3 Transport Routes and Relay Hover Points")
    ax.set_xlabel("Longitude"); ax.set_ylabel("Latitude"); ax.grid(alpha=.25)
...
class JointTimeState:
    # Joint-ALNS 当前完整状态。routes 被保存在状态中，保证路线结构邻域被接受后
    # 后续时间邻域和再次 destroy/repair 都在新的路线结构上继续搜索。
    routes: List[Route]
...
def evaluate_joint_time_state(routes: List[Route], release: Dict[int,float], decoder: ResourceDecoder,
    ...
    try:
        ...
        st=JointTimeState([r.clone() for r in routes],dict(release),decoded,metrics,traj,cands,sorties)
...
def _route_trip_map(state: JointTimeState) -> Dict[int, ScheduledTrip]:
    return {t.route_index:t for t in state.decoded.trips}


def _route_relay_burden(state: JointTimeState) -> Dict[int, float]:
    """各 route 的中继占用秒数，用于定位通信代价高的路线。"""
    trip_to_route={t.trip_id:t.route_index for t in state.decoded.trips}
    burden=defaultdict(float)
    for tid,arr in state.trajectories.items():
        ri=trip_to_route.get(tid)
        if ri is None: continue
        burden[ri]+=COMM_SAMPLE_STEP*sum(1 for x in arr if not x.direct_ok)
    return dict(burden)


def communication_conflict_destroy(routes: List[Route], state: JointTimeState, boxes: Dict[str,Box],
                                   evaluator: RouteEvaluator, rng: random.Random, q: int = 2) -> Tuple[List[Route],List[str],Dict[str,int]]:
    """
    Communication-conflict destroy。

    不随机拆路线，而是优先从“迟到贡献大 + 中继占用长”的路线中移除普通货箱。
    第一版结构邻域保持路线数量不变：每条被破坏路线至少留 1 个货箱，从而 route_index
    与 release time 的索引保持稳定，便于和时间协同邻域混合搜索。
    """
    trip_by_route=_route_trip_map(state)
    relay_burden=_route_relay_burden(state)
    candidates=[]
    for ri,r in enumerate(routes):
        bids=r.box_ids()
        if len(bids)<=1: continue
        tr=trip_by_route.get(ri)
        if tr is None: continue
        late_score=0.0
        removable=[]
        for bid in bids:
            b=boxes[bid]
            td=state.decoded.box_delivery.get(bid,tr.return_time)
            late=b.priority*max(0.0,td-b.expected_time)/max(b.expected_time,1.0)
            late_score+=late
            if b.hard_deadline is None:
                # 只有存在其它物理可行的接收路线时才作为 destroy 候选。
                feasible_elsewhere=False
                for j,rr in enumerate(routes):
                    if j==ri: continue
                    if b.service_id in rr.service_ids() or len(rr.stops)<MAX_STOPS_PER_ROUTE:
                        nr=insert_box_into_route(rr,b,None if b.service_id in rr.service_ids() else len(rr.stops))
                        if evaluator.feasible_types(nr):
                            feasible_elsewhere=True; break
                if feasible_elsewhere:
                    removable.append((late,td/max(b.expected_time,1.0),bid))
        if not removable: continue
        # 中继负担越大、迟到越大越优先破坏；很晚才执行的路线也略加权。
        comm=relay_burden.get(ri,0.0)/600.0
        time_pen=tr.start_time/7200.0
        score=4.0*late_score+0.8*comm+0.25*time_pen
        candidates.append((score,ri,removable))
    if not candidates:
        return [r.clone() for r in routes],[],{}
    candidates.sort(reverse=True,key=lambda z:z[0])
    # 在最差的前 4 条里随机，保留 ALNS 多样性；高分路线仍有更大概率被选择。
    top=candidates[:min(4,len(candidates))]
    weights=[max(1e-6,x[0]) for x in top]
    _,ri,removable=rng.choices(top,weights=weights,k=1)[0]
    removable.sort(reverse=True)
    max_remove=min(max(1,q),len(removable),len(routes[ri].box_ids())-1)
    # 默认只移 1~2 箱，避免一次破坏过大导致通信重新不可行。
    nremove=1 if max_remove==1 else rng.choice([1,max_remove])
    rem=[x[2] for x in removable[:nremove]]
    origin={bid:ri for bid in rem}
    partial=[r.clone() for r in routes]
    for st in partial[ri].stops:
        st.box_ids=[x for x in st.box_ids if x not in set(rem)]
    partial[ri].normalize()
    if not partial[ri].stops:
        return [r.clone() for r in routes],[],{}
    return partial,rem,origin


def relay_aware_repair(partial: List[Route], removed: List[str], origin: Dict[str,int], state: JointTimeState,
                       boxes: Dict[str,Box], evaluator: RouteEvaluator, nodes: Dict[str,Node],
                       comm: CommunicationModel, rng: random.Random) -> Optional[List[Route]]:
    """
    Relay-aware regret repair。

    关键原则：
    1) 优先插入已有相同服务区的早期路线——几乎不改变通信空间几何；
    2) 若必须增加新访问点，则偏好空间邻近、固定网关直连条件较好的路线；
    3) 用当前 Joint 状态中的执行时刻估计迟到，显式奖励“把晚箱前移”；
    4) 不新建路线，保持 route_index 稳定。
    """
    routes=[r.clone() for r in partial]
    pool=set(removed)
    trip_by_route=_route_trip_map(state)
    burden=_route_relay_burden(state)
    while pool:
        best_choice=None
        for bid in list(pool):
            b=boxes[bid]; opts=[]
            src=origin.get(bid,-1)
            bn=nodes[b.service_id]
            service_direct=comm.link_available(Position3D(bn.lon,bn.lat,bn.work_alt),comm.gateway,"TG")
            for i,r in enumerate(routes):
                # 不允许回插到原路线，除非没有其它选项；先记作高罚分备选。
                old=evaluator.proxy_cost(r)
                candidates_for_i=[]
                if b.service_id in r.service_ids():
                    candidates_for_i=[insert_box_into_route(r,b,None)]
                elif len(r.stops)<MAX_STOPS_PER_ROUTE:
                    for pos in range(len(r.stops)+1):
                        candidates_for_i.append(insert_box_into_route(r,b,pos))
                for nr in candidates_for_i:
                    fes=evaluator.feasible_types(nr)
                    if not fes: continue
                    delta_proxy=evaluator.proxy_cost(nr)-old
                    same_service=b.service_id in r.service_ids()
                    # 空间相容性：新增服务区时用与现有停靠点的最短距离做代理。
                    if same_service:
                        spatial=0.0
                    else:
                        spatial=min(haversine_m(bn.lon,bn.lat,nodes[sid].lon,nodes[sid].lat) for sid in r.service_ids())/5000.0
                    tr=trip_by_route.get(i)
                    start=tr.start_time if tr is not None else 0.0
                    # 新路线评价中取能送达该箱的最早 offset，估计修复后的及时性。
                    est=math.inf
                    for ev in fes.values():
                        off=ev.box_delivery_offsets.get(bid,ev.duration)
                        est=min(est,start+off)
                    late_proxy=b.priority*max(0.0,est-b.expected_time)/max(b.expected_time,1.0)
                    hard_pen=0.0
                    if b.hard_deadline is not None and est>b.hard_deadline+1e-9:
                        hard_pen=1000.0
                    relay_pen=0.15*burden.get(i,0.0)/600.0
                    direct_pen=0.0 if service_direct or same_service else 0.35
                    same_bonus=-3.0 if same_service else 0.0
                    earlier_bonus=-0.00018*max(0.0,state.decoded.box_delivery.get(bid,est)-est)
                    source_pen=2.5 if i==src else 0.0
                    score=(delta_proxy+0.75*spatial+2.2*late_proxy+relay_pen+direct_pen+
                           same_bonus+earlier_bonus+source_pen+hard_pen)
                    opts.append((score,i,nr))
            if not opts:
                return None
            opts.sort(key=lambda z:z[0])
            # 若存在非原路线选项，则彻底移除“原样回插”选项。
            non_src=[x for x in opts if x[1]!=src]
            if non_src: opts=non_src
            best=opts[0]; second=opts[1][0] if len(opts)>1 else best[0]+5.0
            regret=second-best[0]
            key=(regret,-best[0],b.priority)
            if best_choice is None or key>best_choice[0]:
                best_choice=(key,bid,best)
        _,bid,(_,idx,nr)=best_choice
        routes[idx]=nr; pool.remove(bid)
    return routes


def structural_joint_candidate(current: JointTimeState, decoder: ResourceDecoder, boxes: Dict[str,Box],
                               nodes: Dict[str,Node], evaluator: RouteEvaluator, dtypes: Dict[str,DroneType],
                               arcs: ArcLibrary, comm: CommunicationModel, rspec: RelaySpec,
                               runits: List[RelayUnit], base: Tuple[float,float,float,int],
                               rng: random.Random) -> Tuple[Optional[JointTimeState],str]:
    """执行一次 communication-conflict destroy + relay-aware repair。"""
    partial,removed,origin=communication_conflict_destroy(current.routes,current,boxes,evaluator,rng,q=2)
    if not removed:
        return None,"no_destroy_candidate"
    repaired=relay_aware_repair(partial,removed,origin,current,boxes,evaluator,nodes,comm,rng)
    if repaired is None:
        return None,"repair_failed:"+"|".join(removed)
    try:
        ensure_route_integrity(repaired,boxes)
    except Exception:
        return None,"integrity_failed"
    cand=evaluate_joint_time_state(repaired,current.release,decoder,boxes,nodes,dtypes,arcs,comm,rspec,runits,base)
    return cand,"move:"+"|".join(removed)
...
def joint_time_alns(initial: JointTimeState, routes: List[Route], decoder: ResourceDecoder,
    ...
    """
    Joint-ALNS v2：时间协同 + communication-conflict destroy + relay-aware repair。

    时间算子负责释放第一阶段为保证通信可行而加入的保守等待；结构算子则把迟到且
    中继代价高的普通货箱从冲突路线中移出，再优先插入通信几何相容的早期路线。
    每个候选都完整重新进行运输资源解码、DEM通信判定、中继位置规划及能源组件调度。
    """
    ...
    if not initial.routes:
        initial.routes=[r.clone() for r in routes]
    current=initial; best=initial
    rng=random.Random(seed+3103)
    hist=[(0,best.score)]
    log=[]
    ops=["conflict_destroy_repair","advance_one","advance_pair","reset_one","exchange_shift"]
    # 结构算子提高初始权重，确保有限迭代下也能真正探索路线结构。
    weights={"conflict_destroy_repair":3.2,"advance_one":1.0,"advance_pair":.8,"reset_one":.8,"exchange_shift":1.2}
    evaluator=decoder.evaluator
    for it in range(1,iterations+1):
        op=rng.choices(ops,weights=[weights[o] for o in ops],k=1)[0]
        detail=""
        if op=="conflict_destroy_repair":
            cand,detail=structural_joint_candidate(current,decoder,boxes,nodes,evaluator,dtypes,arcs,comm,rspec,runits,base,rng)
            if cand is None:
                weights[op]=max(.4,weights[op]*.985)
                hist.append((it,best.score)); log.append([it,op,0,"",best.score,detail]); continue
        else:
            avail=[ri for ri,v in current.release.items() if v>1e-6]
            if not avail:
                hist.append((it,best.score)); log.append([it,op,0,"",best.score,"no_release"]); continue
            ...
            if op=="exchange_shift":
                ...
                if not donor:
                    hist.append((it,best.score)); log.append([it,op,0,"",best.score,"no_donor"]); continue
                ...
                old=rel.get(advance_ri,0.0); new=max(0.0,old-delta)
                if new<=1e-6: rel.pop(advance_ri,None)
                else: rel[advance_ri]=new
                proposed=delay_tr.start_time+delta
                if math.isfinite(delay_tr.route_eval.latest_start): proposed=min(proposed,delay_tr.route_eval.latest_start)
                rel[delay_ri]=max(rel.get(delay_ri,0.0),proposed)
                detail=f"advance r{advance_ri}; delay r{delay_ri}"
            else:
                k=2 if op=="advance_pair" and len(avail)>=2 else 1
                chosen=rng.sample(avail,k)
                for ri in chosen:
                    old=rel.get(ri,0.0)
                    if op=="reset_one": new=0.0
                    else:
                        frac=rng.choice([0.55,0.70,0.82,0.90]); back=rng.choice([120.0,240.0,360.0,600.0])
                        new=max(0.0,min(old*frac,old-back))
                    if new<=1e-6: rel.pop(ri,None)
                    else: rel[ri]=new
                detail="routes:"+"|".join(str(x) for x in chosen)
            cand=evaluate_joint_time_state(current.routes,rel,decoder,boxes,nodes,dtypes,arcs,comm,rspec,runits,base)
            if cand is None:
                weights[op]=max(0.2,weights[op]*0.98)
                hist.append((it,best.score)); log.append([it,op,0,"",best.score,detail]); continue

        temp=max(0.004,0.07*(1-it/max(iterations,1)))
        delta=cand.score-current.score
        accept=(delta<=0) or (rng.random()<math.exp(-delta/temp))
        if accept:
            current=cand; weights[op]*=1.015
        else:
            weights[op]=max(0.2,weights[op]*0.995)
        if cand.score < best.score-1e-10:
            best=cand; weights[op]+=0.20
        hist.append((it,best.score)); log.append([it,op,int(accept),cand.score,best.score,detail])
...
def run_q3(project_root:Path,output_dir:Path,alns_iters:int,joint_iters:int,seed:int)->None:
    ...
    baseline=JointTimeState([r.clone() for r in routes],dict(route_release),decoded,metrics,trajectories,candidates,sorties,1.0)
    ...
    routes=[r.clone() for r in best_joint.routes]; route_release=dict(best_joint.release); decoded=best_joint.decoded; metrics=best_joint.metrics
    ...
    with (output_dir/"Q3_Joint_ALNS日志.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.writer(f); w.writerow(["iteration","operator","accepted","candidate_score","best_score","detail"]); w.writerows(joint_log)
...
def parse_q3_args()->argparse.Namespace:
    ...
    p.add_argument("--joint-iters",type=int,default=20,help="Joint-ALNS v2迭代数（时间协同+通信冲突destroy/relay-aware repair）；建议20~80")
