...
def run_q3(project_root:Path,output_dir:Path,alns_iters:int,seed:int)->None:
    ...
    print("[8/10] 回填官方 Q3 Sheet，并写出详细审计文件...")
...
def parse_q3_args()->argparse.Namespace:
    ...
    p.add_argument("--iters",type=int,default=0,help="第一版可行性基线默认0；非零运输ALNS目前仅用于实验，正式联合ALNS将在下一版加入")
    p.add_argument("--seed",type=int,default=RANDOM_SEED,help="随机种子")
    return p.parse_args()


if __name__=="__main__":
    args=parse_q3_args(); run_q3(Path(args.root).resolve(),Path(args.output).resolve(),max(0,args.iters),args.seed)
