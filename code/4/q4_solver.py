...
也可只用问题三最终输出和模板运行：
    python q4_solver.py --q3-zip output.zip --template 结果提交模板.xlsx --output-dir output_q4
...
def load_node_coordinates(path: Optional[Path]) -> Dict[str, Tuple[float, float]]:
    """尽量从节点文件中识别 O01/Sxxx 的经纬度。"""
    if path is None or not path.exists():
        return {}
    wb = load_workbook(path, read_only=True, data_only=True)
    result: Dict[str, Tuple[float, float]] = {}
    lon_keys = {"经度", "longitude", "lon", "lng"}
    lat_keys = {"纬度", "latitude", "lat"}
    id_keys = {"节点编号", "节点", "编号", "node_id", "id", "服务区编号"}
    for ws in wb.worksheets:
        rows = list(ws.iter_rows(values_only=True))
        if not rows:
            continue
        ...
        for hi in range(min(10, len(rows))):
            hdr = [str(v).strip().lower() if v is not None else "" for v in rows[hi]]
            idx_id = idx_lon = idx_lat = None
            for j, h in enumerate(hdr):
                raw = str(rows[hi][j]).strip() if rows[hi][j] is not None else ""
                if raw in id_keys or h in {x.lower() for x in id_keys}:
                    idx_id = j
                if raw in lon_keys or h in lon_keys:
                    idx_lon = j
                if raw in lat_keys or h in lat_keys:
                    idx_lat = j
            if idx_id is not None and idx_lon is not None and idx_lat is not None:
...
def find_dem_mat(root: Path, explicit: Optional[Path]) -> Optional[Path]:
    if explicit and explicit.exists():
        return explicit
    cands = find_files(root, ["*DEM*.mat", "*dem*.mat", "*.mat", "镇龙乡及周边30米DEM.mat"])
...
def plot_resource_redundancy_compare(reg: FigureRegistry, selected: Dict[int,PartitionEval], pool: Dict[str,int]) -> None:
    ...
    ax.legend(frameon=False); ax.grid(axis="y",alpha=0.18)
    reg.save(fig,"fig_q4_33_resource_redundancy_compare","三种资源共享口径的配置规模比较",
...
def plot_pooled_vs_original_ids(reg: FigureRegistry, data: Q3Data, pool: Dict[str,int]) -> None:
    ...
    ax.legend(frameon=False);ax.grid(axis="y",alpha=0.18)
    reg.save(fig,"fig_q4_34_original_ids_vs_minimum","问题三资源编号与最少共享需求对比",
...
def plot_geo_partition(reg: FigureRegistry, sol: PartitionEval, node_xy: Dict[str,Tuple[float,float]], stem: str,
    ...
    if dem is not None and latv is not None and lonv is not None:
        try:
            extent=[float(np.nanmin(lonv)),float(np.nanmax(lonv)),float(np.nanmin(latv)),float(np.nanmax(latv))]
            ax.imshow(dem,extent=extent,origin="lower",cmap="Greys",alpha=0.23,aspect="auto")
        except Exception:
            pass
    if "O01" in node_xy:
...
def plot_redundancy_decomposition(reg: FigureRegistry, selected: Dict[int,PartitionEval], pool: Dict[str,int]) -> None:
    ...
    ax.legend(frameon=False);ax.grid(axis="y",alpha=0.18)
    reg.save(fig,"fig_q4_40_redundancy_source","主方案资源冗余来源",
...
def generate_all_figures(
    ...
    for K in (2,3):
        ...
        plot_selected_membership(reg,sol,units,f"fig_q4_{35 if K==2 else 36:02d}_K{K}_service_membership")
        plot_replication_matrix(reg,sol,relay_to_units,f"fig_q4_{37 if K==2 else 38:02d}_K{K}_relay_replication")
        plot_relay_copy_energy(reg,sol,data,relay_to_units,f"fig_q4_{41 if K==2 else 42:02d}_K{K}_copy_energy")
        plot_gantt_transport(reg,sol,data,f"fig_q4_{43 if K==2 else 44:02d}_K{K}_transport_gantt")
        plot_gantt_relay(reg,sol,data,f"fig_q4_{45 if K==2 else 46:02d}_K{K}_relay_gantt")
        plot_balance_sensitivity(reg,results[K],K,f"fig_q4_{47 if K==2 else 48:02d}_K{K}_balance_sensitivity")
        plot_top_candidates(reg,results[K],K,f"fig_q4_{49 if K==2 else 50:02d}_K{K}_top_candidates")
        plot_inventory_utilization(reg,sol,inventory,f"fig_q4_{51 if K==2 else 52:02d}_K{K}_inventory_utilization")
        plot_group_box_trip_counts(reg,sol,f"fig_q4_{53 if K==2 else 54:02d}_K{K}_group_task_scale")
        if node_xy:
            plot_geo_partition(reg,sol,node_xy,f"fig_q4_{55 if K==2 else 56:02d}_K{K}_geo_partition",dem,latv,lonv)
            plot_relay_position_map(reg,sol,data,node_xy,f"fig_q4_{57 if K==2 else 58:02d}_K{K}_relay_positions")
...
def write_figure_tex(path: Path, figs: Sequence[FigureInfo]) -> None:
    lines=[
        ...
        "本文档逐一说明程序实际生成的所有问题四图表。每幅图均给出图意、可支持的结论以及是否建议放入正文；未被程序实际生成的可选地理图不会列入本文件。",
        "",
    ]
    def _fig_num(f):
...
def write_reference_tex(path: Path) -> None:
    txt=r"""% 问题四建议参考文献条目（已核对题名、作者和出版信息）
...
def main() -> None:
    ...
    outdir=(args.output_dir or (SCRIPT_PATH.parent/"output_q4")).resolve()
