...
def infer_project_root(script_path: Path) -> Path:
    """
    按本题标准工程结构定位 HUAWEI-CUP 根目录。

    标准放置：
        HUAWEI-CUP/
          code/4/q4_solver_*.py
          数据/...
          结果提交模板.xlsx

    因此程序在 code/4 下时，项目根目录就是 script_path.parents[2]。
    若程序被临时移动，则向上寻找同时含有 code 与 数据 的最近祖先目录。
    最后才退回当前工作目录。
    """
    p = script_path.resolve()
    if len(p.parents) >= 3:
        candidate = p.parents[2]
        if (candidate / "code").is_dir() and (candidate / "数据").is_dir():
            return candidate
    for anc in p.parents:
        if (anc / "code").is_dir() and (anc / "数据").is_dir():
            return anc
    cwd = Path.cwd().resolve()
    if (cwd / "code").is_dir() and (cwd / "数据").is_dir():
        return cwd
    return p.parents[2] if len(p.parents) >= 3 else cwd

DEFAULT_ROOT = infer_project_root(SCRIPT_PATH)

# 用户给出的 HUAWEI-CUP 标准目录结构。优先走这些精确路径；递归搜索仅作兜底。
STD_BASIC_DATA_REL = Path("数据") / "无人机应急物资运输基础数据"
STD_GEO_DATA_REL = Path("数据") / "镇龙乡地理空间数据" / "镇龙乡及周边地理数据"
STD_DEM_REL = STD_GEO_DATA_REL / "数字高程模型数据（DEM）" / "镇龙乡及周边30米DEM.mat"
STD_NODE_REL = STD_BASIC_DATA_REL / "调度中心与服务区.xlsx"
STD_COMM_REL = STD_BASIC_DATA_REL / "通信链路参数.xlsx"
STD_RELAY_REL = STD_BASIC_DATA_REL / "中继无人机数据.xlsx"
STD_TRANSPORT_REL = STD_BASIC_DATA_REL / "运输无人机数据.xlsx"
STD_DEMAND_REL = STD_BASIC_DATA_REL / "物资需求与配送时限.xlsx"
STD_TEMPLATE_REL = Path("结果提交模板.xlsx")
STD_PROBLEM_DOC_REL = Path("山区洪涝灾害下无人机运输与通信协同优化.docx")
STD_GEO_PDF_REL = Path("数据") / "镇龙乡地理空间数据" / "镇龙乡地理空间数据说明.pdf"
STD_BODY_REL = Path("华为杯建模模板") / "华为杯LaTeX模板" / "sections" / "body.tex"
...
def _resolve_explicit_path(root: Path, explicit: Optional[Path]) -> Optional[Path]:
    """显式参数既允许绝对路径，也允许相对于 HUAWEI-CUP 根目录的路径。"""
    if explicit is None:
        return None
...
def find_template(root: Path, explicit: Optional[Path]) -> Optional[Path]:
    p = _resolve_explicit_path(root, explicit)
    if p is not None:
        return p

    # mulu.txt 中模板就在 HUAWEI-CUP 根目录。
    exact = root / STD_TEMPLATE_REL
    if exact.exists() and workbook_has_sheets(exact, {"Q4_分区配置"}):
        return exact.resolve()

    # 兜底：允许带编号/副本名的模板。
    candidates = find_files(root, ["*结果提交模板*.xlsx", "结果提交模板.xlsx"])
    for q in candidates:
        if workbook_has_sheets(q, {"Q4_分区配置"}):
            return q
    return None


def workbook_looks_like_node_table(path: Path) -> bool:
    """
    判断是否为“调度中心与服务区”表。

    注意：O01 与 S001~S015 本来就在不同数据行；上一版错误地要求它们同一行，
    会把真实附件误判为非节点表。本函数跨行累计节点编号后再判定。
    """
    try:
        wb = load_workbook(path, read_only=True, data_only=True)
        ids: Set[str] = set()
        for ws in wb.worksheets:
            # 本题表只有约二十行，但放宽到前 200 行，兼容以后附件格式变化。
            for row in ws.iter_rows(min_row=1, max_row=min(200, ws.max_row), values_only=True):
                for v in row:
                    if v is None:
                        continue
                    s = str(v).strip()
                    if s == "O01" or re.fullmatch(r"S\d{3}", s):
                        ids.add(s)
        wb.close()
        return "O01" in ids and any(re.fullmatch(r"S\d{3}", x) for x in ids)
    except Exception:
        return False
...
def find_node_xlsx(root: Path, explicit: Optional[Path]) -> Optional[Path]:
    ...
    # 最高优先级：严格按照用户 HUAWEI-CUP 目录结构定位。
    exact = root / STD_NODE_REL
    if exact.exists():
        return exact.resolve()

    # 兜底：只在标准基础数据目录中搜索，避免扫到结果表/模板。
    basic_dir = root / STD_BASIC_DATA_REL
    if basic_dir.exists():
        preferred = find_files(basic_dir, ["*调度中心*服务区*.xlsx", "*服务区*.xlsx"])
        for q in preferred:
            if workbook_looks_like_node_table(q):
                return q

    # 最后才对整个项目递归搜索。
    preferred = find_files(root, ["*调度中心*服务区*.xlsx", "*服务区*.xlsx"])
    for q in preferred:
        if "结果" in q.name or "模板" in q.name:
            continue
        if workbook_looks_like_node_table(q):
            return q
    return None
...
def load_node_coordinates(path: Optional[Path]) -> Dict[str, Tuple[float, float]]:
    ...
    try:
        for ws in wb.worksheets:
            ...
            if not rows:
                continue
            # 支持附件中的两个分段表头：第2行“调度中心编号...”，第6行“服务区编号...”。
            for hi in range(min(30, len(rows))):
...
def find_dem_mat(root: Path, explicit: Optional[Path]) -> Optional[Path]:
    ...
    # mulu.txt 中 DEM 的固定位置。
    exact = root / STD_DEM_REL
    if exact.exists():
        return exact.resolve()

    dem_dir = root / STD_GEO_DATA_REL / "数字高程模型数据（DEM）"
    if dem_dir.exists():
        cands = find_files(dem_dir, ["*DEM*.mat", "*dem*.mat"])
        if cands:
            return cands[0]

    cands = find_files(root, ["*DEM*.mat", "*dem*.mat"])
...
def _exact_or_none(root: Path, rel: Path) -> Optional[Path]:
    p = root / rel
    return p.resolve() if p.exists() else None


def discover_named_inputs(root: Path) -> Dict[str, Optional[Path]]:
    """优先按 mulu.txt 的标准相对路径记录上游输入；缺失时再递归兜底。"""
    exact_map: Dict[str, Path] = {
        "problem_docx": STD_PROBLEM_DOC_REL,
        "geo_pdf": STD_GEO_PDF_REL,
        "comm_xlsx": STD_COMM_REL,
        "relay_xlsx": STD_RELAY_REL,
        "transport_xlsx": STD_TRANSPORT_REL,
        "node_xlsx": STD_NODE_REL,
        "demand_xlsx": STD_DEMAND_REL,
        "dem_mat": STD_DEM_REL,
        "body_tex": STD_BODY_REL,
        "template_xlsx": STD_TEMPLATE_REL,
        "q3_solver": Path("code") / "3" / "q3_solver_joint_v2_1.py",
    }
    fallback_patterns = {
        ...
        "q3_solver": ["q3_solver_joint_v2_1.py", "q3_solver*.py"],
        "dem_mat": ["*DEM*.mat", "*dem*.mat"],
        "body_tex": ["body.tex"],
        "template_xlsx": ["*结果提交模板*.xlsx"],
    }
    out: Dict[str, Optional[Path]] = {}
    for key, rel in exact_map.items():
        out[key] = _exact_or_none(root, rel)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--root",type=Path,default=None,help="HUAWEI-CUP项目根目录；默认从 code/4 下脚本位置自动向上定位")
...
def main() -> None:
    args=parse_args()
    root=(args.root.resolve() if args.root is not None else infer_project_root(SCRIPT_PATH))
    outdir=(args.output_dir or (SCRIPT_PATH.parent/"output")).resolve()
    outdir.mkdir(parents=True,exist_ok=True)
    figdir=outdir/"figures"
    figdir.mkdir(parents=True,exist_ok=True)

    print(f"[0/11] 项目根目录：{root}")
    if SCRIPT_PATH.parent.name == "4" and SCRIPT_PATH.parent.parent.name == "code":
        expected = SCRIPT_PATH.parent.parent.parent.resolve()
        if root != expected:
            print(f"  [提示] 脚本位于 code/4，但当前根目录解析为 {root}；标准根目录应为 {expected}")
    ...
    if args.q3_detail and args.q3_detail.exists():
        ...
        # 标准工程中优先从 code/3/output 读取问题三最终详细结果。
        guesses=[
            root/"code"/"3"/"output",
            root/"code"/"3",
            root/"output"/"3",
        ...
        for g in guesses:
            if g.exists():
                ...
                if q3_detail:
                    break
        # 最后才扫描根目录，避免平时无谓遍历整套地理数据。
        if q3_detail is None:
            q3_detail=find_q3_detail_xlsx(root)
    if q3_detail is None:
    ...
    print(f"  节点文件: {node_path if node_path else '未找到'}")
    print(f"  DEM文件 : {dem_path if dem_path else '未找到'}")
